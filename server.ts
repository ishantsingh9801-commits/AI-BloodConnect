...
import { calculateSmartMatchScore, getCompatibleDonorGroups, isCompatible, RBC_DONATION_COMPATIBILITY, RBC_RECIPIENT_COMPATIBILITY } from './server/services/compatibility';
import { parseEmergencyQuery, generateBloodDemandInsights } from './server/services/gemini';
...
app.post('/api/auth/login', (req, res) => {
  ...
  // Prototype demo password validation
  return res.json({
    success: true,
    user,
...
app.patch('/api/requests/:id/cancel', (req, res) => {
  const { id } = req.params;
...
app.post('/api/matching/rank', (req, res) => {
  ...
  const scoredMatches: RankedDonorMatch[] = allDonors
    ...
    // Filter to compatible donors within range
    .filter((m) => m.breakdown.isCompatible && m.breakdown.distanceKm <= maxDistance)
    // Sort descending by 100-point total score
...
app.get('/api/matching/compatibility-table', (req, res) => {
  res.json({
    success: true,
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`AI BloodConnect server running on http://0.0.0.0:${PORT}`);
  });
}
